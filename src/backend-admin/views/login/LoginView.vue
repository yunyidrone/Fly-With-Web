...
function persistRememberAccount() {
  if (rememberPassword.value) {
    localStorage.setItem(
      REMEMBER_KEY,
      JSON.stringify({ username: form.username, password: form.password, savedAt: Date.now() }),
    );
    return;
  }
  localStorage.removeItem(REMEMBER_KEY);
...
async function submit() {
  ...
  try {
    ...
    const redirect = route.query.redirect || MONITOR_BASE;
...
.login-v2 {
  ...
  --login-hint-size: clamp(11px, calc(14 / var(--design-h) * 100vh), 16px);
  --login-hint-gap: calc(30 / var(--design-h) * 100vh);
